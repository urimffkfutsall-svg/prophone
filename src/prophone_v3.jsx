...
function Dashboard({ data, setData, onNavigate, currentWorker, T }) {
  ...
  const __revResetKey = "prophone_revreset_" + (data.business?.id || "x"); const __revResetAt = (function(){ try { const v = localStorage.getItem(__revResetKey); return v ? new Date(v).getTime() : 0; } catch(e){ return 0; } })(); const __revWindowStart = Math.max(__revResetAt, Date.now() - 86400000); const totalRevenue = data.jobs.filter(j => j.status === "perfunduar" && j.price && new Date(j.createdAt).getTime() >= __revWindowStart).reduce((sum, j) => sum + parseFloat(j.price || 0), 0);
  ...
  return (
    <div>
      ...
      {/* Revenue summary */}
      <div className="summary-cards" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 12, marginBottom: 16 }}>
        {[
          ...
          { label: "Te ardhura 24h", val: `${totalRevenue.toFixed(2)}€`, color: "#8B5CF6" },
        ].map(item => (
      ...
      <div style={{ background: T.surface, borderRadius: 20, padding: 24, border: `1.5px solid ${T.border}` }}>
        ...
        <div className="job-row-header" style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr 1fr 1fr 1.5fr 0.8fr 0.8fr 1.2fr 0.8fr", padding: "10px 16px", fontSize: 12, fontWeight: 700, color: T.textMuted, textTransform: "uppercase", letterSpacing: 0.5, borderBottom: `2px solid ${T.border}` }}>
        ...
        {filteredJobs.length === 0 ? (
          ...
          return (
            <div key={job.id} className="job-row" style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr 1fr 1fr 1.5fr 0.8fr 0.8fr 1.2fr 0.8fr", padding: "14px 16px", fontSize: 13, alignItems: "center", borderBottom: `1px solid ${T.border}`, cursor: "pointer", transition: "background .15s", background: stale ? `${T.danger}08` : "transparent" }}
...
function Icon({ name, size, color }) {
  ...
  const p = { width: sz, height: sz, viewBox: "0 0 24 24", fill: "none", stroke: cl, strokeWidth: 1.75, strokeLinecap: "round", strokeLinejoin: "round" };
  switch(name) {
    ...
    case "cart": return <svg {...p}><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 2h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg>;
    ...
    case "card": return <svg {...p}><rect x="1" y="4" width="22" height="16" rx="2" ry="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>;
    ...
    case "phone": return <svg {...p}><rect x="5" y="2" width="14" height="20" rx="2" ry="2"/><line x1="12" y1="18" x2="12.01" y2="18"/></svg>;
...
function Landing({ T, onLogin, onRegister }) {
  const goLogin = () => { try { window.__authMode = "login"; } catch(e){ } onLogin(); };
  const goRegister = () => { try { window.__authMode = "register"; } catch(e){ } onRegister(); };
  const S = {
    page: { minHeight: "100vh", background: T.bg, color: T.text, fontFamily: "-apple-system, BlinkMacSystemFont, Inter, Segoe UI, Roboto, sans-serif", lineHeight: 1.5 },
    nav: { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 24px", borderBottom: "1px solid " + T.border, background: T.surface, position: "sticky", top: 0, zIndex: 100, backdropFilter: "blur(12px)" },
    logoWrap: { display: "flex", alignItems: "center", gap: 8 },
    logoDot: { width: 10, height: 10, borderRadius: 3, background: T.accent },
    logo: { fontSize: 20, fontWeight: 800, color: T.text, letterSpacing: -0.5 },
    navBtns: { display: "flex", gap: 8, alignItems: "center" },
    btnGhost: { display: "inline-flex", alignItems: "center", gap: 6, padding: "9px 16px", borderRadius: 10, border: "1px solid " + T.border, background: "transparent", color: T.text, fontWeight: 600, fontSize: 14, cursor: "pointer" },
    btnPrimary: { display: "inline-flex", alignItems: "center", gap: 6, padding: "9px 16px", borderRadius: 10, border: "none", background: T.accent, color: "#fff", fontWeight: 700, fontSize: 14, cursor: "pointer" },
    hero: { padding: "80px 24px 56px", textAlign: "center", maxWidth: 980, margin: "0 auto" },
    pill: { display: "inline-flex", alignItems: "center", gap: 8, padding: "6px 14px", borderRadius: 999, background: T.surfaceAlt, border: "1px solid " + T.border, fontSize: 13, color: T.textMuted, fontWeight: 600, marginBottom: 24 },
    pillDot: { width: 6, height: 6, borderRadius: "50%", background: T.accent, display: "inline-block" },
    h1: { margin: "0 0 20px", fontSize: 56, fontWeight: 900, lineHeight: 1.05, letterSpacing: -1.5, color: T.text },
    h1grad: { background: T.accentGrad || T.accent, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" },
    heroSub: { margin: "0 auto 32px", fontSize: 19, color: T.textMuted, maxWidth: 680, lineHeight: 1.55 },
    heroBtns: { display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap", marginBottom: 12 },
    btnHeroPrimary: { display: "inline-flex", alignItems: "center", gap: 8, padding: "16px 28px", borderRadius: 12, border: "none", background: T.accent, color: "#fff", fontWeight: 700, fontSize: 16, cursor: "pointer", boxShadow: "0 10px 30px -10px " + T.accent },
    btnHeroGhost: { display: "inline-flex", alignItems: "center", gap: 8, padding: "16px 28px", borderRadius: 12, border: "1px solid " + T.border, background: T.surface, color: T.text, fontWeight: 700, fontSize: 16, cursor: "pointer" },
    miniNote: { fontSize: 13, color: T.textFaint, marginTop: 12 },
    statsRow: { display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24, maxWidth: 880, margin: "48px auto 0", padding: "32px 24px", borderTop: "1px solid " + T.border, borderBottom: "1px solid " + T.border },
    statItem: { textAlign: "center" },
    statNum: { fontSize: 32, fontWeight: 900, color: T.text, letterSpacing: -0.5 },
    statLabel: { fontSize: 12, color: T.textMuted, fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.8, marginTop: 4 },
    section: { padding: "80px 24px", maxWidth: 1140, margin: "0 auto" },
    sectionHeader: { textAlign: "center", marginBottom: 48 },
    eyebrow: { fontSize: 13, fontWeight: 700, color: T.accent, textTransform: "uppercase", letterSpacing: 1.5, marginBottom: 12 },
    sectionTitle: { fontSize: 36, fontWeight: 900, margin: "0 0 12px", color: T.text, letterSpacing: -0.8, lineHeight: 1.15 },
    sectionSub: { fontSize: 17, color: T.textMuted, margin: "0 auto", maxWidth: 600, lineHeight: 1.55 },
    modulesGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 16 },
    moduleCard: { background: T.surface, border: "1px solid " + T.border, borderRadius: 16, padding: 24, transition: "all 0.2s" },
    moduleIconBox: { width: 48, height: 48, borderRadius: 12, background: T.surfaceAlt, color: T.accent, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 16 },
    moduleName: { fontSize: 16, fontWeight: 700, color: T.text, marginBottom: 6 },
    moduleDesc: { fontSize: 14, color: T.textMuted, lineHeight: 1.5 },
    featureRow: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 48, alignItems: "center", marginBottom: 80 },
    featureEyebrow: { fontSize: 13, fontWeight: 700, color: T.accent, textTransform: "uppercase", letterSpacing: 1.5, marginBottom: 12 },
    featureTitle: { fontSize: 30, fontWeight: 800, margin: "0 0 16px", color: T.text, letterSpacing: -0.5, lineHeight: 1.2 },
    featureText: { fontSize: 16, color: T.textMuted, lineHeight: 1.65, margin: "0 0 20px" },
    featureList: { listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: 12 },
    featureLi: { display: "flex", alignItems: "flex-start", gap: 10, fontSize: 15, color: T.text },
    featureLiIcon: { color: T.accent, flexShrink: 0, marginTop: 2, display: "inline-flex" },
    featureVisual: { background: T.surface, border: "1px solid " + T.border, borderRadius: 20, padding: 24 },
    mockCard: { background: T.surfaceAlt, borderRadius: 12, padding: 14, marginBottom: 10, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 },
    mockLeft: { display: "flex", alignItems: "center", gap: 12, minWidth: 0 },
    mockIcon: { width: 36, height: 36, borderRadius: 10, background: T.surface, color: T.accent, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 },
    mockText: { fontSize: 14, fontWeight: 600, color: T.text },
    mockSub: { fontSize: 12, color: T.textMuted, marginTop: 2 },
    mockBadge: { padding: "4px 10px", borderRadius: 999, background: T.accent, color: "#fff", fontSize: 11, fontWeight: 700, whiteSpace: "nowrap" },
    valuesGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 16 },
    valueCard: { background: T.surface, border: "1px solid " + T.border, borderRadius: 16, padding: 28 },
    valueIconBox: { width: 44, height: 44, borderRadius: 12, background: T.surfaceAlt, color: T.accent, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 16 },
    valueTitle: { fontSize: 18, fontWeight: 700, color: T.text, marginBottom: 6 },
    valueDesc: { fontSize: 14, color: T.textMuted, lineHeight: 1.55 },
    ctaBox: { background: T.accentGrad || T.accent, borderRadius: 24, padding: "64px 32px", textAlign: "center", color: "#fff" },
    ctaTitle: { fontSize: 36, fontWeight: 900, margin: "0 0 12px", color: "#fff", letterSpacing: -0.8, lineHeight: 1.15 },
    ctaSub: { fontSize: 17, opacity: 0.95, margin: "0 auto 28px", color: "#fff", maxWidth: 520 },
    ctaBtn: { display: "inline-flex", alignItems: "center", gap: 8, padding: "16px 32px", borderRadius: 12, border: "none", background: "#fff", color: T.accent, fontWeight: 800, fontSize: 16, cursor: "pointer" },
    footer: { padding: "48px 24px 32px", borderTop: "1px solid " + T.border, background: T.surface },
    footerInner: { maxWidth: 1140, margin: "0 auto", display: "grid", gridTemplateColumns: "2fr 1fr 1fr", gap: 32 },
    footerLogoRow: { display: "flex", alignItems: "center", gap: 8, marginBottom: 12 },
    footerTag: { fontSize: 14, color: T.textMuted, lineHeight: 1.55, maxWidth: 320 },
    footerColTitle: { fontSize: 13, fontWeight: 700, color: T.text, textTransform: "uppercase", letterSpacing: 1, marginBottom: 12 },
    footerLine: { fontSize: 14, color: T.textMuted, marginBottom: 8 },
    footerLink: { cursor: "pointer", color: T.textMuted, fontSize: 14, display: "block", marginBottom: 8 },
    footerBottom: { maxWidth: 1140, margin: "32px auto 0", paddingTop: 24, borderTop: "1px solid " + T.border, textAlign: "center", fontSize: 13, color: T.textFaint }
  };
  const modules = [
    { icon: "wrench", name: "Servisi", desc: "Riparime telefonash me ndjekje statusi dhe kuponat publike" },
    { icon: "cart", name: "Arka POS", desc: "Shitje te shpejta, faturat A4 dhe inventari" },
    { icon: "package", name: "Posta", desc: "Porosi dhe dorezime me ndjekje online" },
    { icon: "users", name: "Klientet", desc: "Baza e plote e klienteve me histori" },
    { icon: "user", name: "Punetoret", desc: "Stafi, detyrat dhe performancat" },
    { icon: "shield", name: "Garancite", desc: "Garanci profesionale A4 me logon" },
    { icon: "card", name: "Borxhet", desc: "Menaxho borxhet e klienteve me lehtesi" },
    { icon: "chart", name: "Raportet", desc: "Te ardhura ditore, mujore dhe vjetore" }
  ];
  return (
    <div style={S.page}>
      <style>{`
        @media (max-width: 768px) {
          .ld-hero { padding: 56px 20px 40px !important; }
          .ld-h1 { font-size: 38px !important; letter-spacing: -1px !important; }
          .ld-hero-sub { font-size: 16px !important; }
          .ld-stats { gap: 12px !important; padding: 24px 16px !important; margin-top: 32px !important; }
          .ld-stat-num { font-size: 24px !important; }
          .ld-section { padding: 56px 20px !important; }
          .ld-section-title { font-size: 28px !important; letter-spacing: -0.5px !important; }
          .ld-feature-row { grid-template-columns: 1fr !important; gap: 24px !important; margin-bottom: 56px !important; }
          .ld-feature-title { font-size: 24px !important; }
          .ld-cta-box { padding: 48px 24px !important; }
          .ld-cta-title { font-size: 28px !important; }
          .ld-footer-inner { grid-template-columns: 1fr !important; gap: 24px !important; }
          .ld-nav-btns button { padding: 8px 12px !important; font-size: 13px !important; }
          .ld-hero-btn { padding: 14px 22px !important; font-size: 15px !important; }
        }
      `}</style>
      <nav style={S.nav}>
        <div style={S.logoWrap}><div style={S.logoDot}></div><div style={S.logo}>DataPos</div></div>
        <div style={S.navBtns} className="ld-nav-btns">
          <button onClick={goLogin} style={S.btnGhost}>Kyqu</button>
          <button onClick={goRegister} style={S.btnPrimary}>Regjistrohu<Icon name="arrow" size={16} color="#fff"/></button>
        </div>
      </nav>
      <section style={S.hero} className="ld-hero">
        <div style={S.pill}><span style={S.pillDot}></span><span>Tani me sinkronizim online</span></div>
        <h1 style={S.h1} className="ld-h1">Menaxhoni biznesin tuaj<br/>me <span style={S.h1grad}>DataPos</span></h1>
        <p style={S.heroSub} className="ld-hero-sub">Platforma e plote per servisin e telefonave, arken, postat dhe te gjitha operacionet ditore - ne nje vend te vetem.</p>
        <div style={S.heroBtns}>
          <button onClick={goRegister} style={S.btnHeroPrimary} className="ld-hero-btn">Regjistrohu Falas<Icon name="arrow" size={18} color="#fff"/></button>
          <button onClick={goLogin} style={S.btnHeroGhost} className="ld-hero-btn">Kyqu</button>
        </div>
        <div style={S.miniNote}>30 dite falas . Pa karte krediti . Anulo ne cdo kohe</div>
        <div style={S.statsRow} className="ld-stats">
          <div style={S.statItem}><div style={S.statNum} className="ld-stat-num">500+</div><div style={S.statLabel}>Biznese</div></div>
          <div style={S.statItem}><div style={S.statNum} className="ld-stat-num">50K+</div><div style={S.statLabel}>Pune</div></div>
          <div style={S.statItem}><div style={S.statNum} className="ld-stat-num">99.9%</div><div style={S.statLabel}>Uptime</div></div>
        </div>
      </section>
      <section style={S.section} className="ld-section">
        <div style={S.sectionHeader}>
          <div style={S.eyebrow}>MODULET</div>
          <h2 style={S.sectionTitle} className="ld-section-title">Cdo gje qe ju nevojitet</h2>
          <p style={S.sectionSub}>Tete module te integruara per te menaxhuar cdo aspekt te biznesit tuaj me lehtesi.</p>
        </div>
        <div style={S.modulesGrid}>
          {modules.map(m => <div key={m.name} style={S.moduleCard}><div style={S.moduleIconBox}><Icon name={m.icon} size={22} color={T.accent}/></div><div style={S.moduleName}>{m.name}</div><div style={S.moduleDesc}>{m.desc}</div></div>)}
        </div>
      </section>
      <section style={S.section} className="ld-section">
        <div style={S.featureRow} className="ld-feature-row">
          <div>
            <div style={S.featureEyebrow}>SERVISI</div>
            <h3 style={S.featureTitle} className="ld-feature-title">Ndjekje profesionale e riparimeve</h3>
            <p style={S.featureText}>Regjistroni pune te reja, ndiqni statuset dhe gjeneroni kuponat publike per klientet - gjithcka brenda 30 sekondash.</p>
            <ul style={S.featureList}>
              <li style={S.featureLi}><span style={S.featureLiIcon}><Icon name="check" size={18} color={T.accent}/></span>Link publik per klientin ne kohe reale</li>
              <li style={S.featureLi}><span style={S.featureLiIcon}><Icon name="check" size={18} color={T.accent}/></span>Kuponat e printueshem me logon tuaj</li>
              <li style={S.featureLi}><span style={S.featureLiIcon}><Icon name="check" size={18} color={T.accent}/></span>Statuset dhe njoftimet automatike</li>
            </ul>
          </div>
          <div style={S.featureVisual}>
            <div style={S.mockCard}><div style={S.mockLeft}><div style={S.mockIcon}><Icon name="phone" size={18} color={T.accent}/></div><div><div style={S.mockText}>iPhone 14 Pro</div><div style={S.mockSub}>IMEI: 35178...</div></div></div><div style={S.mockBadge}>Ne pune</div></div>
            <div style={S.mockCard}><div style={S.mockLeft}><div style={S.mockIcon}><Icon name="phone" size={18} color={T.accent}/></div><div><div style={S.mockText}>Samsung S23</div><div style={S.mockSub}>IMEI: 35892...</div></div></div><div style={S.mockBadge}>Perfunduar</div></div>
            <div style={S.mockCard}><div style={S.mockLeft}><div style={S.mockIcon}><Icon name="phone" size={18} color={T.accent}/></div><div><div style={S.mockText}>Xiaomi 13</div><div style={S.mockSub}>IMEI: 35441...</div></div></div><div style={S.mockBadge}>Pranuar</div></div>
          </div>
        </div>
        <div style={S.featureRow} className="ld-feature-row">
          <div style={S.featureVisual}>
            <div style={S.mockCard}><div style={S.mockLeft}><div style={S.mockIcon}><Icon name="cart" size={18} color={T.accent}/></div><div><div style={S.mockText}>Shitje #1245</div><div style={S.mockSub}>3 produkte</div></div></div><div style={S.mockBadge}>120 EUR</div></div>
            <div style={S.mockCard}><div style={S.mockLeft}><div style={S.mockIcon}><Icon name="card" size={18} color={T.accent}/></div><div><div style={S.mockText}>Borxh - Klient A</div><div style={S.mockSub}>Afati: 15 dite</div></div></div><div style={S.mockBadge}>45 EUR</div></div>
            <div style={S.mockCard}><div style={S.mockLeft}><div style={S.mockIcon}><Icon name="shield" size={18} color={T.accent}/></div><div><div style={S.mockText}>Garanci #G102</div><div style={S.mockSub}>Vlefshme 12 muaj</div></div></div><div style={S.mockBadge}>Aktive</div></div>
          </div>
          <div>
            <div style={S.featureEyebrow}>ARKA & BORXHET</div>
            <h3 style={S.featureTitle} className="ld-feature-title">Shitje te shpejta, gjithcka e organizuar</h3>
            <p style={S.featureText}>Arka POS me skanim te shpejte, menaxhim produktesh, faturat A4, garancite dhe borxhet e klienteve - te sinkronizuara online.</p>
            <ul style={S.featureList}>
              <li style={S.featureLi}><span style={S.featureLiIcon}><Icon name="check" size={18} color={T.accent}/></span>Faturat profesionale A4 me logon</li>
              <li style={S.featureLi}><span style={S.featureLiIcon}><Icon name="check" size={18} color={T.accent}/></span>Menaxhim borxhesh me afate</li>
              <li style={S.featureLi}><span style={S.featureLiIcon}><Icon name="check" size={18} color={T.accent}/></span>Garancite e printueshme</li>
            </ul>
          </div>
        </div>
        <div style={S.featureRow} className="ld-feature-row">
          <div>
            <div style={S.featureEyebrow}>RAPORTET</div>
            <h3 style={S.featureTitle} className="ld-feature-title">Statistikat ne kohe reale</h3>
            <p style={S.featureText}>Ndiqni te ardhurat ditore, mujore dhe vjetore. Resetoni numeruesin kur deshironi dhe ruani raportet per analiza te metejshme.</p>
            <ul style={S.featureList}>
              <li style={S.featureLi}><span style={S.featureLiIcon}><Icon name="check" size={18} color={T.accent}/></span>Te ardhurat 24h te dukshme ne Ballina</li>
              <li style={S.featureLi}><span style={S.featureLiIcon}><Icon name="check" size={18} color={T.accent}/></span>Raporte ditore, mujore, vjetore</li>
              <li style={S.featureLi}><span style={S.featureLiIcon}><Icon name="check" size={18} color={T.accent}/></span>Ruajtje online e raporteve</li>
            </ul>
          </div>
          <div style={S.featureVisual}>
            <div style={S.mockCard}><div style={S.mockLeft}><div style={S.mockIcon}><Icon name="chart" size={18} color={T.accent}/></div><div><div style={S.mockText}>Te ardhura 24h</div><div style={S.mockSub}>+12% nga dje</div></div></div><div style={S.mockBadge}>340 EUR</div></div>
            <div style={S.mockCard}><div style={S.mockLeft}><div style={S.mockIcon}><Icon name="chart" size={18} color={T.accent}/></div><div><div style={S.mockText}>Te ardhura mujore</div><div style={S.mockSub}>Maj 2026</div></div></div><div style={S.mockBadge}>8420 EUR</div></div>
            <div style={S.mockCard}><div style={S.mockLeft}><div style={S.mockIcon}><Icon name="chart" size={18} color={T.accent}/></div><div><div style={S.mockText}>Te ardhura vjetore</div><div style={S.mockSub}>2026</div></div></div><div style={S.mockBadge}>62800 EUR</div></div>
          </div>
        </div>
      </section>
      <section style={S.section} className="ld-section">
        <div style={S.sectionHeader}>
          <div style={S.eyebrow}>PERFITIMET</div>
          <h2 style={S.sectionTitle} className="ld-section-title">Pse DataPos?</h2>
          <p style={S.sectionSub}>I ndertuar per bizneset e vogla dhe te mesme qe duan te punojne me shpejt dhe me sigurt.</p>
        </div>
        <div style={S.valuesGrid}>
          <div style={S.valueCard}><div style={S.valueIconBox}><Icon name="zap" size={22} color={T.accent}/></div><div style={S.valueTitle}>Shpejt dhe i thjeshte</div><div style={S.valueDesc}>Hapni dhe perdorni menjehere. Pa konfigurim te komplikuar, gati per pune ne minuta.</div></div>
          <div style={S.valueCard}><div style={S.valueIconBox}><Icon name="shield" size={22} color={T.accent}/></div><div style={S.valueTitle}>I sigurt</div><div style={S.valueDesc}>Te dhenat e ruajtura online me enkriptim te plote dhe rezerva automatike.</div></div>
          <div style={S.valueCard}><div style={S.valueIconBox}><Icon name="globe" size={22} color={T.accent}/></div><div style={S.valueTitle}>Online & Mobile</div><div style={S.valueDesc}>Qasje kudo qe jeni - desktop, telefon ose tablet. Sinkronizim ne kohe reale.</div></div>
        </div>
      </section>
      <section style={S.section} className="ld-section">
        <div style={S.ctaBox} className="ld-cta-box">
          <h2 style={S.ctaTitle} className="ld-cta-title">Filloni sot me DataPos</h2>
          <p style={S.ctaSub}>Provoni 30 dite falas, pa kerkesa per karte krediti. Anuloni kur te doni.</p>
          <button onClick={goRegister} style={S.ctaBtn}>Regjistrohu Falas<Icon name="arrow" size={18} color={T.accent}/></button>
        </div>
      </section>
      <footer style={S.footer}>
        <div style={S.footerInner} className="ld-footer-inner">
          <div>
            <div style={S.footerLogoRow}><div style={S.logoDot}></div><div style={S.logo}>DataPos</div></div>
            <div style={S.footerTag}>Platforma e plote per menaxhimin e biznesit - servisi, arka, posta, klientet dhe me shume.</div>
          </div>
          <div>
            <div style={S.footerColTitle}>Kontakt</div>
            <div style={S.footerLine}>Tel: 045 278 279</div>
            <div style={S.footerLine}>info@datapos.pro</div>
          </div>
          <div>
            <div style={S.footerColTitle}>Llogaria</div>
            <span onClick={goLogin} style={S.footerLink}>Kyqu</span>
            <span onClick={goRegister} style={S.footerLink}>Regjistrohu</span>
          </div>
        </div>
        <div style={S.footerBottom}>{"Copyright \u00A9 2026 DataPos . Te gjitha te drejtat e rezervuara"}</div>
      </footer>
    </div>
  );
}
...
function RevenueResetSection({ data, T }) {
  const RESET_KEY = "prophone_revreset_" + (data.business?.id || "x");
  const showToast = (msg) => { try { window.alert(msg); } catch(e){ } };
  const getReset = () => { try { const v = localStorage.getItem(RESET_KEY); return v ? new Date(v).getTime() : 0; } catch(e){ return 0; } };
  const completed = data.jobs.filter(j => j.status === "perfunduar" && j.price);
  const now = new Date();
  const dayStart = Math.max(getReset(), now.getTime() - 86400000);
  const monthStart = new Date(now.getFullYear(), now.getMonth(), 1).getTime();
  const yearStart = new Date(now.getFullYear(), 0, 1).getTime();
  const sumIn = (s) => completed.filter(j => new Date(j.createdAt).getTime() >= s).reduce((a, j) => a + parseFloat(j.price || 0), 0);
  const cntIn = (s) => completed.filter(j => new Date(j.createdAt).getTime() >= s).length;
  const ditore = sumIn(dayStart), mujore = sumIn(monthStart), vjetore = sumIn(yearStart);
  const [reports, setReports] = useState([]);
  const [busy, setBusy] = useState(false);
  const loadReports = async () => {
    if (!data.business?.id) return;
    try {
      const res = await supabase.from("revenue_reports").select("*").eq("account_id", data.business.id).order("created_at", { ascending: false }).limit(50);
      if (res && res.data) setReports(res.data);
    } catch(e){ }
  };
  useEffect(() => { loadReports(); }, [data.business?.id]);
  const doReset = async (periodType, label, amount, jobsCount, periodStart) => {
    if (busy) return;
    if (!data.business?.id) { showToast("Hyr ne llogari fillimisht"); return; }
    if (!window.confirm("Reseto te ardhurat " + label + "? Totali (" + amount.toFixed(2) + " EUR) do te ruhet te raportet.")) return;
    setBusy(true);
    try {
      const res = await supabase.from("revenue_reports").insert({ account_id: data.business.id, period_type: periodType, label: label, amount: amount, jobs_count: jobsCount, period_start: new Date(periodStart).toISOString(), period_end: new Date().toISOString() });
      if (res.error) { showToast("Gabim ne ruajtje: " + (res.error.message || "")); setBusy(false); return; }
      try { localStorage.setItem(RESET_KEY, new Date().toISOString()); } catch(e){ }
      showToast(label + " u ruajt te raportet dhe u resetua");
      await loadReports();
    } catch(e){ showToast("Gabim ne lidhje"); }
    setBusy(false);
  };
  const fmtNum = (n) => (parseFloat(n) || 0).toFixed(2) + " EUR";
  const fmtDt = (s) => { try { return new Date(s).toLocaleString("sq-AL"); } catch(e){ return ""; } };
  return (
    <div style={{ background: T.surface, borderRadius: 20, padding: 24, marginBottom: 20, border: "1.5px solid " + T.border }}>
      <h3 style={{ margin: "0 0 4px", fontSize: 16, fontWeight: 700, color: T.text }}>Reseto dhe raporto te ardhurat</h3>
      <p style={{ margin: "0 0 16px", fontSize: 13, color: T.textMuted }}>Ruaj totalin te raportet dhe rifillo numeruesin e Ballines (24h).</p>
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 16 }}>
        <div style={{ flex: "1 1 130px", background: T.surfaceAlt, borderRadius: 12, padding: 14, textAlign: "center", border: "1px solid " + T.border }}>
          <div style={{ fontSize: 12, color: T.textMuted, marginBottom: 4 }}>Ditore (24h)</div>
          <div style={{ fontSize: 20, fontWeight: 800, color: T.accent }}>{ditore.toFixed(2)} EUR</div>
        </div>
        <div style={{ flex: "1 1 130px", background: T.surfaceAlt, borderRadius: 12, padding: 14, textAlign: "center", border: "1px solid " + T.border }}>
          <div style={{ fontSize: 12, color: T.textMuted, marginBottom: 4 }}>Mujore</div>
          <div style={{ fontSize: 20, fontWeight: 800, color: T.accent }}>{mujore.toFixed(2)} EUR</div>
        </div>
        <div style={{ flex: "1 1 130px", background: T.surfaceAlt, borderRadius: 12, padding: 14, textAlign: "center", border: "1px solid " + T.border }}>
          <div style={{ fontSize: 12, color: T.textMuted, marginBottom: 4 }}>Vjetore</div>
          <div style={{ fontSize: 20, fontWeight: 800, color: T.accent }}>{vjetore.toFixed(2)} EUR</div>
        </div>
      </div>
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
        <button disabled={busy} onClick={() => doReset("daily", "Ditore", ditore, cntIn(dayStart), dayStart)} style={{ flex: "1 1 130px", background: T.accent, color: "#fff", border: "none", borderRadius: 10, padding: "12px 14px", cursor: busy ? "default" : "pointer", fontWeight: 700, fontSize: 13, opacity: busy ? 0.5 : 1 }}>Reseto Ditore</button>
        <button disabled={busy} onClick={() => doReset("monthly", "Mujore", mujore, cntIn(monthStart), monthStart)} style={{ flex: "1 1 130px", background: T.accent, color: "#fff", border: "none", borderRadius: 10, padding: "12px 14px", cursor: busy ? "default" : "pointer", fontWeight: 700, fontSize: 13, opacity: busy ? 0.5 : 1 }}>Reseto Mujore</button>
        <button disabled={busy} onClick={() => doReset("yearly", "Vjetore", vjetore, cntIn(yearStart), yearStart)} style={{ flex: "1 1 130px", background: T.accent, color: "#fff", border: "none", borderRadius: 10, padding: "12px 14px", cursor: busy ? "default" : "pointer", fontWeight: 700, fontSize: 13, opacity: busy ? 0.5 : 1 }}>Reseto Vjetore</button>
      </div>
      <h4 style={{ margin: "20px 0 8px", fontSize: 14, fontWeight: 700, color: T.text }}>Raporte te ruajtura</h4>
      {reports.length === 0 ? <div style={{ fontSize: 13, color: T.textFaint, padding: "8px 0" }}>Nuk ka raporte ende.</div> : <div>{reports.map(r => <div key={r.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 12px", borderBottom: "1px solid " + T.border, fontSize: 13 }}><span style={{ color: T.textMuted }}>{r.label} - {fmtDt(r.created_at)}</span><span style={{ fontWeight: 700, color: T.text }}>{fmtNum(r.amount)}</span></div>)}</div>}
    </div>
  );
}
...
const PostaFieldInput = React.memo(function PostaFieldInput({ label, field, type, placeholder, required, value, onChange, T }) {
  return (
  ...
  return (
    <div style={{ background: T.surface, borderRadius: 20, padding: 24, border: `1.5px solid ${T.border}` }}>
      ...
      <div className="posta-form-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
        <PostaFieldInput T={T} label="Emri" field="clientName" required placeholder="p.sh. Arben" value={form.clientName} onChange={set} />
        <PostaFieldInput T={T} label="Mbiemri" field="clientSurname" placeholder="p.sh. Krasniqi" value={form.clientSurname} onChange={set} />
        <PostaFieldInput T={T} label="Telefon" field="clientPhone" placeholder="+383..." value={form.clientPhone} onChange={set} />
        <PostaFieldInput T={T} label="Qyteti" field="city" required placeholder="p.sh. Prishtinë" value={form.city} onChange={set} />
        <PostaFieldInput T={T} label="Shteti" field="country" placeholder="p.sh. Kosovë" value={form.country} onChange={set} />
        <PostaFieldInput T={T} label="Adresa" field="address" placeholder="Rruga, Nr..." value={form.address} onChange={set} />
        <PostaFieldInput T={T} label="Përshkrim" field="description" placeholder="Produkt..." value={form.description} onChange={set} />
        ...
        <nav style={{ background: T.nav, borderBottom: `1px solid ${T.border}`, padding: "14px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 100 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 24 }}>
            ...
            <div className="desktop-nav-tabs" style={{ display: "flex", gap: 4 }}>
              {(() => { let n = [NAV_BASE[0], NAV_BASE[1], NAV_BASE[2]]; if (data.business?.hasArka) n.push(NAV_ARKA); if (data.business?.hasPosta) n.push(NAV_POSTA); n.push({ key: "borgjet", label: "Borgjet" }); n.push(NAV_BASE[3]); return n; })().map(item => (
